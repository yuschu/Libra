...
### 时空分类规则（必须统一遵守）
- **时间粒度**：所有流量/指标统一按**小时**聚合，时间戳对齐到整点
- **时间维度**：小时(0–23)、星期(周一~周日)、工作日/周末、高峰时段（早高峰7–9、平峰、晚高峰17–19、夜间）四类视图
- **空间单元**：最小单元为路段 `road_id`，可向上聚合到区域；路段需配经纬度/边界坐标用于地图
- **空间亮块规则**：地图上按路段/区域上色，颜色按拥堵等级（畅通绿、缓行黄、拥堵橙、严重红），图例统一
- **时空叠加**：选定时间范围 → 地图按该时段聚合值亮块；可叠加事故、车/非机动车类型等图层，图层开关独立
...
- 傻瓜式网页上传页（自动识别列映射、清洗、入库并反馈条数/坏行；**支持多文件一键批量导入**）
- 启动选择「示例数据 / 导入数据」、无数据空态弹窗、一键清空数据
- 建全表：`traffic_flow` / `accident` / `vehicle_type` / `behavior`
- 多源数据管理：列出已接入的数据源、各自状态与条数，可查看、可清空

**阶段 2 · 可视化大屏**
- 路段下拉切换、时段×星期拥堵热力图
- **3D 地球仪总览**：地图总览做成地球仪形式（ECharts GL），拥堵/流量色块在地球仪上按位置亮显；点击具体位置时相机做**飞行动画切换**到该区域
- **时间轴播放**：提供时间轴，可拖动或自动播放，色块随时间逐帧动画变化，直观展示拥堵随时间的演变
- **时空叠加分析**：时间轴/时段筛选与地图联动——选某时段，地图同步亮块；可叠加事故、车/非机动车类型等图层
- 事故、车/非机动车类型、交通行为图表
- 大屏风格、地图、动效
