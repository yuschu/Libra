...
- 非专业人员**无需装环境、无需懂命令**：做到一键启动；最终交付为双击即看的形式（**打包由本人完成**，opencode 不必做打包）
- 仍要求：不依赖外网（演示现场没网也能跑）
- **区分两个版本**：
  - 开发版：完整保留技术细节——结构化日志、`.env` 配置、接口文档、目录分层、README 都要齐全，便于开发与维护
  - 产品版（最终交付）：才是黑盒——只展示界面，不暴露命令行/代码/技术实现，用户不用懂怎么实现
...
## 十三、执行清单（按阶段顺序做；每完成一步 commit+push，等 review 通过再进下一步）

**阶段 0 · 基础加固**
- 统一响应体封装、`.env` 配置、CORS、结构化日志到 `logs/`
- 后端按 `core/ routers/ services/ models/ schemas/` 分层
- 关键接口最小测试

**阶段 1 · 数据层**
- 多格式预处理 pipeline（CSV/Excel/JSON/API，可插拔 adapter）
- 傻瓜式网页上传页（自动识别列映射、清洗、入库并反馈条数/坏行）
- 启动选择「示例数据 / 导入数据」、无数据空态弹窗、一键清空数据
- 建全表：`traffic_flow` / `accident` / `vehicle_type` / `behavior`
...
- 产品版（黑盒、打包）由本人最后完成，opencode 不用做
